...
public class 재고확인_수정 {
	...
	public static void main(String[] args) {
		...
		while (true) {
			...
			if (stockChoice == 1) {
				System.out.println(" -----재고를 조회할 파트를 고르세요----- ");
				for (int i = 0; i < categories.length; i++) {
					System.out.println((i + 1) + ":" + categories[i]);
				}
			} else if (stockChoice == 2) {
				System.out.println(" -----재고를 수정할 파트를 고르세요----- ");
				for (int i = 0; i < categories.length; i++) {
					System.out.println((i + 1) + ":" + categories[i]);
				}
			}

			int checkStock = sc.nextInt();
			if (checkStock >= 1) {
				int categoryIndex = checkStock - 1;
				if (categoryIndex >= 0 && categoryIndex < categories.length) {
					...
					System.out.println("재고를 수정할 화장품 번호를 선택하세요:");
					for (int i = 0; i < selectedStock.length; i++) {
					...
					if (typeIndex >= 1 && typeIndex <= selectedStock.length) {
						...
						System.out.println("유효하지 않은 종류 번호입니다.");
					}
				}
